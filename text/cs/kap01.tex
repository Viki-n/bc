...
kvantifikovat citlivost pouze pomocí hit rate (tedy poměru počtu hitů k součtu
počtů hitů a missů) není vhodné -- napčíklad pozorovatel, který vždy odpoví
...
Při detekci signálu rozeznáváme dva druhy šumu. Šum vnitřní a šum vnější.
...
Praxe ukazuje, že v těchto situacích je rozdělení veličiny $X$ dobře
aproximováno normálním rozdělením, a to jak v případě, že signál přítomen je,
tak v případě, že přítomen není. Obě tato normální rozdělení navíc mají stejný
rozptyl a liší se jen svou střední hodnotou.
